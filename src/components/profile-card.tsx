...
export function ProfileCard({ profile, onConnect, onReadMore }: ProfileCardProps) {
    ...
    return (
        ...
        >
            <CardContent className="p-4 md:p-6 lg:p-6 flex-1 flex flex-col">
                    ...
                    <div className="flex gap-3 md:gap-3 lg:gap-4 flex-1">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center justify-between gap-2 md:gap-2 lg:gap-2">
                                <h3 className="font-bold text-xl md:text-xl lg:text-2xl">{profile.name}</h3>
                                {profile.linkedin && (
                                    <a
                ...
                {profile.reason && (
                    <div className="mt-3 md:mt-4 lg:mt-4 p-3 md:p-4 lg:p-4 bg-muted/30 rounded-lg border border-border/50">
                        <p className="font-semibold text-foreground/80 mb-2 md:mb-2 lg:mb-2 text-xs md:text-xs lg:text-xs uppercase tracking-wide">Why this match</p>
                        <p className="text-sm md:text-sm lg:text-sm text-muted-foreground leading-relaxed md:leading-relaxed lg:leading-relaxed">
                            {(() => {
                                const reasonText = isExpanded || profile.reason.length <= 200 
                                    ? profile.reason 
            ...
            <CardFooter className="bg-muted/20 p-4 md:p-4 lg:p-4 flex justify-end">
                <Button size="default" className="text-sm md:text-sm lg:text-sm px-6 md:px-6 lg:px-6 py-2 md:py-2 lg:py-2" onClick={handleRequestIntroClick}>
