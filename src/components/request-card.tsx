...
export function RequestCard({
  ...
  // API returns created_at as already formatted relative time (e.g., "about 2 hours ago")
  // So we use it directly instead of parsing as Date
  const timeAgo = request.timestamp || "Unknown";
          ...
          <div className="flex-1 space-y-4">
            ...
            <div className="bg-muted/30 rounded-lg p-3 relative">
              <div className="absolute top-3 right-3">
                <Badge
                  variant="secondary"
                  className="text-[10px] h-5 bg-background/50 backdrop-blur-sm"
                >
                  {request.requester.historyCount > 0
                    ? `Helped ${request.requester.historyCount}x`
                    : "New Request"}
                </Badge>
              </div>
