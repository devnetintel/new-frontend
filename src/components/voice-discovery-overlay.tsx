...
import { Mic, X, MessageSquare, Sparkles, ChevronRight, Search, Edit2, ArrowRight, Keyboard, ArrowUp, Check } from "lucide-react"
...
export function VoiceDiscoveryInline({ isActive, onClose, onSearch, initialQuery, selectedNetworks = [] }: VoiceDiscoveryInlineProps) {
    ...
    // Mock conversation flow - using useCallback to prevent recreating on every render
    ...
    const simulateConversation = useCallback(async (userInput: string) => {
        ...
        if (currentStep === 0) {
            newQuery = "Performance marketing professionals with big budget experience"
            systemResponse = "Got it! To help narrow this down: Are you looking for people who've managed budgets above $1M annually? And are you specifically interested in those with experience in e-commerce, SaaS, or consumer apps?"
        } else if (currentStep === 1) {
            newQuery = "Performance marketing | B2C E-commerce | $5M+ budgets"
            systemResponse = "Perfect. One last thing: Are you looking for people currently in leadership roles, or would you also want to connect with freelancers and consultants?"
        } else {
            newQuery = "Performance marketing leaders in B2C E-commerce with $5M+ annual ad spend, in-house teams"
            systemResponse = "Great! I've refined your search based on our conversation."
        }

        // Update ref immediately
        conversationStepRef.current = nextStep

        // Update states separately - NO nesting
    ...
    const handleTextSubmit = () => {
        if (textInput.trim()) {
            simulateConversation(textInput)
        }
    }
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            simulateConversation("At my company I handle performance marketing and I've got a big budget, so I want to talk with people who had handled big budgets for performance marketing in a B2C scenario.")
        }, 3500)

        return () => clearTimeout(timer)
    }, [isListening, simulateConversation])
    ...
    useEffect(() => {
        ...
        if (initialQuery && initialQuery.trim() && !hasStartedRef.current) {
            ...
            setTimeout(() => {
                simulateConversation(initialQuery)
            }, 300)
        }
    }, [isActive, initialQuery]) // Removed simulateConversation from deps
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {selectedNetworks.length > 0 ? (
                            ...
                            <span className="text-xs text-muted-foreground">No networks selected</span>
                        )}
                    </div>
                    ...
                    <div className="bg-muted/30 border border-border/50 rounded-xl p-4">
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                                <p className="text-xs text-muted-foreground mb-1">Refining your search...</p>
                                <p className="text-sm font-medium text-foreground">{refinedQuery}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <>
                        ...
                        {messages.length === 0 && !isListening && (
                            <div className="flex flex-col items-center justify-center text-center space-y-6 py-12">
                                ...
                                <div>
                                    <h3 className="text-2xl font-semibold mb-2">How can we help you?</h3>
                                    <p className="text-sm text-muted-foreground max-w-md">
                                        I'll ask a few questions to understand exactly what you're looking for.
                                    </p>
                                </div>
                            </div>
                        )}
