...
export default function Dashboard() {
  ...
  // Fetch workspaces (for display purposes, but user role is determined by hub dashboard API)
  useEffect(() => {
  ...
  useEffect(() => {
    const loadHubData = async () => {
      ...
      try {
        ...
        setUserState(dashboardData.is_hub_user ? "active_hub" : "new_spoke");
        setHubDashboard(dashboardData.stats);
        setConnectionsMade(dashboardData.stats?.connections_made || 0);

        // Only fetch requests if user is actually a hub user
        if (dashboardData.is_hub_user) {
          console.log("User is hub user, fetching pending requests...");
          try {
            ...
            const transformedRequests: RequestData[] =
              requestsData.requests.map((req: HubRequest) => ({
          ...
          console.log("User is not a hub user, skipping request fetch");
        }
      } catch (error) {
  ...
  // Separate effect to fetch requests when userState is active_hub
  // This ensures requests are fetched even if state is manually changed
  useEffect(() => {
    const fetchRequestsForHub = async () => {
      if (userState !== "active_hub" || !isSignedIn || !user?.id || !isLoaded) {
        return;
      }
      ...
      try {
        ...
        const token = await getToken();
        if (!token) {
          console.error("No token available for pending requests API call");
          setIsLoadingHubData(false);
          return;
        }

        console.log("Fetching pending requests for hub user...");
        const requestsData = await fetchHubPendingRequests(token, {
          limit: 50,
          offset: 0,
        });
    ...
    fetchRequestsForHub();
  }, [userState, isLoaded, isSignedIn, user?.id, getToken]);
  ...
  const handleSendIntro = async (
    ...
    try {
      ...
      setConnectionsMade((prev) => Math.max(0, prev - 1));
      toast.error(
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="outline"
                size="sm"
                className="h-7 md:h-8 text-xs md:text-sm border-dashed border-yellow-500/50 text-yellow-500 hover:text-yellow-400 hover:bg-yellow-500/10 hidden md:flex shrink-0"
              >
        ...
        {userState === "new_spoke" ? (
            ...
            <CardContent className="p-4 md:p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="space-y-1 min-w-0 flex-1">
                ...
                <p className="text-xs md:text-sm text-muted-foreground">
                  Upload your contacts to start generating Magic Links and
                  helping your friends find intros.
                </p>
              </div>
              <div className="flex flex-col sm:flex-row gap-2 md:gap-3 shrink-0">
                ...
                <Button
                  variant="default"
                  className="gap-2 text-xs md:text-sm h-8 md:h-10"
                  onClick={() => setUserState("active_hub")}
                >
                  <Upload className="h-3.5 w-3.5 md:h-4 md:w-4" />
                  Upload CSV
                </Button>
          ...
          <div className="space-y-8">
            <ImpactHeader
              ...
              activeRequests={
                hubDashboard?.active_requests || hubRequests.length
              }
                        ...
                        <div className="flex items-center gap-2 md:gap-4 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0 overflow-hidden">
                            ...
                            <p className="text-[10px] md:text-xs text-muted-foreground/70 mt-0.5 truncate">
                              via {request.workspace.owner_name}&apos;s Network
                              • {timeAgo}
                            </p>
                          </div>
                        </div>
                          ...
                          <div className="flex flex-col gap-4">
                              ...
                              <div className="flex flex-col items-center pt-0.5">
                                ...
                                <div
                                  className={`w-0.5 h-10 mt-1 ${
                                    isHubApproved ? "bg-primary" : "bg-border"
                                  }`}
                                />
                              </div>
                            ...
                            <div className="flex items-start gap-3">
                              <div className="flex flex-col items-center pt-0.5">
                                ...
                                <div
                                  className={`w-0.5 h-10 mt-1 ${
                                    isS2Connected ? "bg-primary" : "bg-border"
                                  }`}
                                />
                              </div>
                              <div className="flex-1 pt-0.5">
